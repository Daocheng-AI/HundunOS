// hundunos/kernel/config/config-loader-v2.test.js
// 配置加载器单元测试

import { describe, it, expect, beforeEach, afterEach } from 'vitest';
import { ConfigLoader, GlobalConfig, KernelConfig, ModelRouterConfig } from './config-loader-v2.js';
...
describe('ConfigLoader', () => {
  ...
  describe('load', () => {
    it('should load config with schema values when env vars are set', () => {
      ...
      expect(config.restApi.port).toBe(39000);
      expect(config.configSource).toBe('mixed');
    });
    ...
    it('should merge schema and JSON configs', () => {
      process.env.NODE_ENV = 'development';
      process.env.HUNDUNOS_LOG_LEVEL = 'debug';

      const config = ConfigLoader.load('.', 'development');

      expect(config.configSource).toBe('mixed' || 'schema');
      expect(config.kernel.logLevel).toBe('debug');
    });
  });
...
describe('KernelConfig', () => {
  it('should create a valid kernel config instance', () => {
    const config = KernelConfig.create();
...
describe('ModelRouterConfig', () => {
  it('should create a valid model router config instance', () => {
    const config = ModelRouterConfig.create();

    expect(config.defaultStrategy).toBeDefined();
    expect(config.localFirst).toBeDefined();
    expect(config.endpoint).toBeDefined();
    expect(config.localModel).toBeDefined();
    expect(config.circuitBreaker).toBeDefined();
    expect(config.openai).toBeDefined();
    expect(config.anthropic).toBeDefined();
    expect(config.glm).toBeDefined();
  });
});
