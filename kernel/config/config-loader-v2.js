// hundunos/kernel/config/config-loader-v2.js
// 基于 Schema 的配置加载器（不使用装饰器）

import { readFileSync, existsSync } from 'fs';
import { join } from 'path';
import { fileURLToPath } from 'url';
import { GlobalConfig } from './schema-config.js';
...
export class ConfigLoader {
  /**
   * 加载配置（优先使用 Schema 配置，回退到 JSON 配置）
  ...
  static load(baseDir, env = null) {
    ...
    // 1. 加载 Schema 配置
    const schemaConfig = this._loadSchemaConfig(environment);

    // 2. 加载 JSON 配置（用于向后兼容和默认值）
    const jsonConfig = this._loadJsonConfig(baseDir, environment);

    // 3. 合并配置（Schema 配置优先）
    const merged = this._mergeConfigs(schemaConfig, jsonConfig);

    // 4. 设置元数据
    merged.environment = environment;
    merged.loadedAt = new Date().toISOString();
    merged.configSource = this._determineConfigSource(schemaConfig, jsonConfig);
   ...
   */
  static _loadSchemaConfig(environment) {
    try {
      const config = GlobalConfig.create();
      return config;
    } catch (error) {
      console.warn(`[Config] Failed to load schema config: ${error.message}`);
      return {};
    }
  }
  ...
  /**
   * 合并 Schema 配置和 JSON 配置
   * @param {Object} schemaConfig - Schema 配置
   * @param {Object} jsonConfig - JSON 配置
   * @returns {Object} 合并后的配置
   */
  static _mergeConfigs(schemaConfig, jsonConfig) {
    // 深度合并，Schema 配置优先
    return this._deepMerge({}, jsonConfig, schemaConfig);
  }
   ...
   * @returns {string} 配置来源
   */
  static _determineConfigSource(schemaConfig, jsonConfig) {
    const hasSchemaConfig = Object.keys(schemaConfig).length > 0;
    const hasJsonConfig = Object.keys(jsonConfig).length > 0;

    if (hasSchemaConfig && hasJsonConfig) {
      return 'mixed';
    } else if (hasSchemaConfig) {
      return 'schema';
  ...
  static getConfigStats(config) {
    return {
      ...
      hasSchemaConfig: config.configSource === 'schema' || config.configSource === 'mixed',
