...
console.log('导出', result.items.length, '条缓存');
```
...
// 获取统计信息
const stats = cache.getStats();
console.log('内存缓存大小:', stats.size);
console.log('持久化缓存大小:', stats.persistentStats.size);
```
...
console.log('导出', exportResult.items.length, '条缓存');
