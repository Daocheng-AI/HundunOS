...
console.log('同步状态:', status.status);
console.log('上次同步:', status.lastSyncTime);
```
...
console.log('冲突数量:', stats.conflictsDetected);
```
...
syncManager.on('conflict:resolved', (conflict) => {
  console.log('冲突已解决:', conflict.resolution);
});
```
...
if (result.success) {
  console.log('同步成功:', result.task.stats);
} else {
