...
export class TodoManager {
  ...
  update(items) {
    ...
    for (const index of items) {
      const content = String(items[index]?.content || '').trim();
      const status = String(items[index]?.status || 'pending').toLowerCase();
      const activeForm = String(items[index]?.activeForm || '').trim();

      if (!content) {
        throw new Error(`Item ${index}: content is required`);
      }
