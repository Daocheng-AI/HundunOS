...
export class RateLimiter {
    ...
    check(req, res) {
        ...
        if (!bucket) {
            // P1-4 修复：新桶初始令牌数为 burstMax（突发容量），而非 maxRequests（速率上限）
            // 这样客户端首次访问时可以短时间内发出 burstMax 个请求，之后按 maxRequests/windowMs 速率补充
            bucket = { tokens: this.burstMax, lastRefill: now };
        ...
        if (tokensToAdd > 0) {
            // 补充上限为 burstMax（突发容量），不能超过桶的最大容量
            bucket.tokens = Math.min(this.burstMax, bucket.tokens + tokensToAdd);
    ...
    _setHeaders(res, bucket, now, allowed) {
        ...
        // P2-6 修复：RFC 7231 规定 Retry-After 仅应在 429/503 响应中出现；
        // 请求被允许时不应发送该响应头（原 Retry-After: 0 语义错误）
        if (!allowed) {
