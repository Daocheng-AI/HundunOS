...
export class ExpressionSandbox {
  ...
  async execute(expression, context = {}) {
    ...
    try {
      ...
      if (!expression || expression.trim() === '') {
        return ExpressionResult.success(undefined, { 
  ...
  /**
   * 在沙箱中执行表达式
   * 安全修复：使用 Node.js 内置 vm 模块替代已废弃的 vm2（CVE-2023-29017 等）
   * vm2 已于 2023 年停止维护，存在多个未修复的沙箱逃逸漏洞。
   * 此处改用 Node.js 内置 vm.runInNewContext() + 严格上下文控制实现等效隔离。
   */
  async _executeInSandbox(expression, context) {
    // 动态导入内置 vm 模块（ESM 兼容写法）
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        // 创建受控沙箱上下文（Object.create(null) 切断原型链）
        const sandboxObj = this._createSandbox(context);
        const sandboxCtx = vm.createContext(Object.assign(Object.create(null), sandboxObj));

        // 在独立上下文中运行，不暴露宿主 global/process/require
        const result = vm.runInContext(`(${expression})`, sandboxCtx, {
  ...
  _createSandbox(context) {
    const sandbox = {
      ...
      // 添加 setTimeout 和 clearTimeout 以支持超时测试
      setTimeout,
      clearTimeout,
      setInterval,
      clearInterval,
      ...
      ...(this.config.allowConsole ? {
        console: {
          log: (...args) => { console.log('[Sandbox]', ...args); return true; },
