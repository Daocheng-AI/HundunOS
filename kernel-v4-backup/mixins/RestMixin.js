...
export const RestMixin = class RestMixin {
    ...
    async init_rest_api(kernel) {
        ...
        // SubagentManager
        try {
            const { SubagentManager, TaskMode, IsolationLevel } = await import(_r('kernel/subagents/index.js'));
    ...
    async init_hooks(kernel) {
        try {
            const { HookExecutor, HookEvent, createDefaultHooks } = await import(_r('kernel/hooks/index.js'));
            const { HookLoader } = await import(_r('kernel/hooks/hook-loader.js'));
