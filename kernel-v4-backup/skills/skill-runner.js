...
export class SkillRunner {
    ...
    async _execShell(tool, params, timeout, isolation) {
        ...
        if (!this._validateCommand(cmd)) {
            throw new Error('SEC-05: Command injection detected. Command contains forbidden patterns.');
        }

        if (isolation === 'process') {
            // SEC-05: 使用参数化调用替代 shell: true
            // 将命令拆分为参数数组，避免 shell 解析
            const cmdParts = this._parseCommandSafe(cmd);
