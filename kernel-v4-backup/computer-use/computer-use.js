...
class WindowsBackend extends ComputerBackend {
  ...
  _sanitizeNumber(value, name = 'value') {
  ...
  _sanitizeString(value, name = 'value') {
  ...
  async moveMouse(x, y) {
    const sx = this._sanitizeNumber(x, 'x');
    const sy = this._sanitizeNumber(y, 'y');
    const ps = `[System.Windows.Forms.Cursor]::Position = New-Object System.Drawing.Point(${sx}, ${sy})`;
    const result = this._runPs(ps);
  ...
  async typeText(text) {
    const safeText = this._sanitizeString(text, 'text');
    const escaped = safeText.replace(/[{}+^~()']/g, c => `{${c}}`);
  ...
  async pressKey(key) {
    const safeKey = this._sanitizeString(key, 'key').replace(/[^a-zA-Z0-9]/g, '');
    const ps = `Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.SendKeys]::SendWait('{${safeKey}}')`;
    const result = this._runPs(ps);
    return { success: !!result };
  }
  ...
  async windowFocus(pid) {
    const safePid = this._sanitizeNumber(pid, 'pid');
    const ps = `(Get-Process -Id ${safePid}).MainWindowHandle | Set-Focus`;
    const result = this._runPs(ps);
    return { success: !!result };
  }
}
...
class DarwinBackend extends ComputerBackend {
  ...
  async moveMouse(x, y) {
    try {
      const sx = Number(x);
      const sy = Number(y);
      if (!Number.isFinite(sx) || !Number.isFinite(sy)) throw new Error('Invalid coordinates');
      execSync(`cliclick m:${sx},${sy}`, { timeout: 2000 });
      return { success: true };
    } catch { return { success: false }; }
  }

  async click(button = 'left') {
    const btn = button === 'right' ? 'r' : (button === 'left' ? 'c' : 'c');
    try {
  ...
  async typeText(text) {
    try {
      ...
      execSync(`cliclick t:'${safeText}'`, { timeout: 2000 });
      return { success: true };
    } catch { return { success: false }; }
  }
}
