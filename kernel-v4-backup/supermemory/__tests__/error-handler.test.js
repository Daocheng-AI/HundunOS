...
describe('ErrorHandler', () => {
  ...
  describe('构造函数', () => {
    it('应该使用默认配置', () => {
      ...
      expect(defaultHandler.config.delay).toBe(100);
  ...
  describe('classifyError', () => {
    ...
    it('应该分类未知错误', () => {
      ...
      expect(classification.retryable).toBe(true); // 未知错误默认可重试
    });
  });
  ...
  describe('retry', () => {
    ...
    it('应该在失败时重试直到成功', async () => {
      ...
      const result = await errorHandler.retry(fn, { delay: 10 });
      expect(result).toBe('success');
    ...
    it('应该在达到最大重试次数后抛出错误', async () => {
      ...
      await expect(
        errorHandler.retry(fn, { delay: 10 })
