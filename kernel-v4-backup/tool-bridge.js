 ...
 */
export function maybePersistedOutput(toolUseId, output, kernel) {
...
export class ToolBridge {
    constructor(kernel) {
        ...
        this.pythonPath = envManager.get('HUNDUNOS_PYTHON', kernel?.config?.system?.platform?.pythonPath || 'python');

        // 支持环境变量和 system.json 配置工具路径
        this.toolsDir = this._resolveToolsDir(
            envManager.get('HUNDUNOS_TOOLS_DIR', kernel?.config?.system?.toolBridge?.toolsDir)
        );
        ...
        this._rustEnabled = envManager.getBoolean('HUNDUNOS_RUST_BRIDGE', false) ||
    ...
    async initialize() {
        ...
        // review: removed console.log(`[ToolBridge] ${this.tools.size} tools (${available} available) in ${this.categories.size} categories`);

        // Phase 4: Rust 执行后端状态
        if (this.rustAdapter) {
            // review: removed console.log('[ToolBridge] Rust backend: ✅ adapters/rust-modules/tool (hundunos-core daemon)');
        } else if (this._rustEnabled) {
            // review: removed console.log('[ToolBridge] Rust backend: ⚠️  legacy subprocess mode (no daemon)');
        } else {
            // review: removed console.log('[ToolBridge] Rust backend: disabled');
        }
    }
    ...
    _discoverTools() {
        ...
        if (discovered > 0) {
            // review: removed console.log(`[ToolBridge] File-level discovery: ${discovered} tools registered from ${this.toolsDir}`);
        }
    }
    ...
    _rustBinaryPath() {
        const candidates = [
            envManager.get('HUNDUNOS_RUST_BINARY'),
    ...
    async _initRustBridge() {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.rustBridge.on('close', (code) => {
                // review: removed console.log(`[ToolBridge/Rust] Process exited (code ${code})`);
                this.rustBridge = null;
            });
